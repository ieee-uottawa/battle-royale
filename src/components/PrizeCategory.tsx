...
export default function PrizeCategory({ category }: { category: PrizeCat }) {
  return (
    <div className="overflow-hidden rounded-lg bg-gray-900 border border-gray-800">
      <div className={`rounded-b-lg p-4 ${category.color}`}>
        <img
          src={category.logo}
          alt={category.title}
          className="w-32 h-16 mx-auto object-contain my-4"
        />
        {(category.title == "Game Jam" ||
          category.title == "(2 rounds)" ||
          category.title == "Bonus Prizes") && (
        ...
        {category.prizes.map((prize, index) => (
          <div
            key={index}
            className="mb-2 last:mb-0 border border-gray-300 border-opacity-10 rounded-xl py-4 px-2 hover:border-opacity-50 transition-all duration-300"
          >
            {prize.place ? (
